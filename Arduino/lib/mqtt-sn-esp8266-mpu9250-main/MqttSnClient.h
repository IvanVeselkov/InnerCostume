...
public:
    ...
    bool publish(char *payload, char *topic_name, int8_t qos) {
      ...
      // check if topic_name is already registered
        ...
        mqttSnMessageHandler.send_publish(&gw_address, (uint8_t *) payload, payload_length, msg_id, topic_id, topic_type,
                                          false, qos, false);
        if (qos > 0) {
            while (this->await_msg_type != MQTTSN_PINGREQ) {
                // wait until we have no other messages in flight
...
bool publish(uint8_t *payload, char *topic_name, int8_t qos) {

        // check if payload is not too long
        uint16_t payload_length = 0;
        uint8_t msg_publish_without_payload_length = 7;
        if (sizeof(payload) > socketInterface.getMaximumMessageLength() - msg_publish_without_payload_length) {
            // payload is too long
            return false;
        }
        payload_length = 4 * sizeof(payload);
        if (qos == 2) {
            // TODO not implemented yet
            return false;
        }
    ...
    bool publish(uint8_t *payload,  uint16_t predef_topic_id, int8_t qos) {
